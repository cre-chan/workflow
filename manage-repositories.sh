...
github_checks() {
  ...
  gh api "repos/$repo/installation" --jq '.app_slug' | grep -qx workflow-codex || {
...
deploy_repository() {
  ...
  for source_file in \
    ...
    .github/ISSUE_TEMPLATE/agentic-story.yml; do
  ...
  bash -n "$checkout"/*.sh "$checkout/.github/scripts"/*.sh
  bash "$checkout/tests/validate-patch-test.sh" >/dev/null 2>&1 || {
    [[ ! -f "$checkout/tests/validate-patch-test.sh" ]] || { echo "$repo: validation failed; no commit was made" >&2; exit 1; }
  }
  (cd "$checkout" && git add -- \
    .github/workflows/codex-agent-ready.yml \
    .github/scripts/validate-and-apply-patch.sh \
...
case "$action" in
  ...
  status)
    if docker compose ps --status running --services | grep -qx runner-manager; then
      docker compose exec -T runner-manager status
    else
